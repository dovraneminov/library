...
export default function MyPublisher({ publishers }) {
  ...
  function openInfo() {
    dispatch(authorInfoAction(publishers.id));
    navigate(`/publisher/${publishers?.id}`);
  }

  return (
    <div className="card border border-0 mb-3" style={{ width: '18rem' }}>
      <img onClick={() => openInfo()} src={publishers.photo} className="card-img-top rounded" alt="..." style={{ boxShadow: '1px 0px 5px grey', height: '260px' }} />
